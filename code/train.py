...
train_align_collate = AlignCollate('training', ts.MAX_N_OBJECTS, ts.MEAN, ts.STD, ts.IMAGE_HEIGHT, ts.IMAGE_WIDTH,
                                   random_horizontal_flipping=ts.HORIZONTAL_FLIPPING, random_vertical_flipping=ts.VERTICAL_FLIPPING,
                                   random_90x_rotation=ts.ROTATION_90X,
                                   random_rotation=ts.ROTATION, random_color_jittering=ts.COLOR_JITTERING,
...
test_align_collate = AlignCollate('test', ts.MAX_N_OBJECTS, ts.MEAN, ts.STD, ts.IMAGE_HEIGHT, ts.IMAGE_WIDTH,
                                  random_horizontal_flipping=ts.HORIZONTAL_FLIPPING, random_vertical_flipping=ts.VERTICAL_FLIPPING,
                                  random_90x_rotation=ts.ROTATION_90X,
                                  random_rotation=ts.ROTATION, random_color_jittering=ts.COLOR_JITTERING,
                                  use_coordinates=ts.USE_COORDINATES)
...
# Define Model
model = Model(ts.MAX_N_OBJECTS, use_instance_segmentation=ts.USE_INSTANCE_SEGMENTATION,
              use_coords=ts.USE_COORDINATES, load_model_path=opt.model, usegpu=opt.usegpu)

# Train Model
model.fit(ts.CRITERION, ts.DELTA_VAR, ts.DELTA_DIST, ts.NORM, ts.LEARNING_RATE, ts.WEIGHT_DECAY, ts.CLIP_GRAD_NORM,
          ts.LR_DROP_FACTOR, ts.LR_DROP_PATIENCE, ts.OPTIMIZE_BG, ts.OPTIMIZER,
          ts.TRAIN_CNN, opt.nepochs, ts.CLASS_WEIGHTS, train_loader, test_loader, model_save_path, opt.debug)
