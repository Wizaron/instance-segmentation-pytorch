...
SEMANTIC_ANN_DIR = os.path.join(DATA_DIR, 'processed', 'cityscapes', 'semantic-annotations')
INSTANCE_ANN_DIR = os.path.join(DATA_DIR, 'processed', 'cityscapes', 'instance-annotations')
...
for _i, subset in enumerate(SUBSETS):
    semantic_ann_paths_all = glob.glob(os.path.join(SEMANTIC_ANN_DIR, subset, '*.npy'))
    ...
    for f in semantic_ann_paths_all:
        ...
        semantic_ann_path = f
        instance_ann_path = os.path.join(INSTANCE_ANN_DIR, subset, name + '.npy')
        img_path = os.path.join(IMG_DIR, subset, _dir, name + '_leftImg8bit.png')
    ...
    create_dataset(out_path, image_paths, semantic_ann_paths, instance_ann_paths)
